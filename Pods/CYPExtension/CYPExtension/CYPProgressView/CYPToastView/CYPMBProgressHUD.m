...
CGFloat const CYPMBProgressMaxOffset = 1000000.f;

static const CGFloat CYPMBDefaultPadding = 16.f;
static const CGFloat CYPMBDefaultLabelFontSize = 16.f;
static const CGFloat CYPMBDefaultDetailsLabelFontSize = 12.f;


@interface CYPMBProgressHUD () {
    // Deprecated
...
@interface CYPMBProgressHUDRoundedButton : UIButton
@end


@implementation CYPMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    CYPMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    CYPMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (CYPMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (CYPMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = CYPMBProgressHUDAnimationFade;
    _mode = CYPMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(CYPMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == CYPMBProgressHUDAnimationZoom) {
        type = animatingIn ? CYPMBProgressHUDAnimationZoomIn : CYPMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == CYPMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == CYPMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == CYPMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == CYPMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    CYPMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<CYPMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    CYPMBBackgroundView *backgroundView = [[CYPMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = CYPMBProgressHUDBackgroundStyleSolidColor;
    ...
    CYPMBBackgroundView *bezelView = [CYPMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:CYPMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:CYPMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [CYPMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:CYPMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[CYPMBRoundProgressView class]];

    CYPMBProgressHUDMode mode = self.mode;
    if (mode == CYPMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == CYPMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[CYPMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == CYPMBProgressHUDModeDeterminate || mode == CYPMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[CYPMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == CYPMBProgressHUDModeAnnularDeterminate) {
            [(CYPMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == CYPMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == CYPMBProgressHUDModeText) {
        [indicator removeFromSuperview];
//        [self.button removeTarget:nil action:NULL forControlEvents:UIControlEventAllEvents];
        indicator = nil;
    }
    else if (mode == CYPMBProgressHUDModeRotationView && self.customView != indicator) {
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self rotViewWithView:indicator];
        [self.bezelView addSubview:indicator];
    }
...
- (void)rotViewWithView:(UIView *)view {
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[CYPMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[CYPMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[CYPMBRoundProgressView class]]) {
        CYPMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [CYPMBRoundProgressView appearanceWhenContainedIn:[CYPMBProgressHUD class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[CYPMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((CYPMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((CYPMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[CYPMBBarProgressView class]]) {
        CYPMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [CYPMBBarProgressView appearanceWhenContainedIn:[CYPMBProgressHUD class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[CYPMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((CYPMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((CYPMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    CYPMBBackgroundView *bezelView = self.bezelView;
...
- (void)updateConstraints {
    ...
    NSDictionary *metricsTOP = @{@"marginTop": @(marginTop)};
    ...
    [self addConstraints:centeringConstraints];
        ...
        if (!CGSizeEqualToSize(maxSize, CGSizeZero)) {
            ...
            CGFloat margin1 = (screen.bounds.size.width - maxSize.width)/2.0;
            NSDictionary *metrics1 = @{@"margin1": @(margin1)};
            NSMutableArray *sideConstraints = [NSMutableArray array];
            [sideConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=margin1)-[bezel]-(>=margin1)-|" options:0 metrics:metrics1 views:NSDictionaryOfVariableBindings(bezel)]];
    ...
    if (self.square) {
        NSLayoutConstraint *square = [NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:bezel attribute:NSLayoutAttributeWidth multiplier:1.0f constant:0];
        square.priority = 997.f;
        [bezelConstraints addObject:square];
    }

    // Top and bottom spacing
    [topSpacer addConstraint:[NSLayoutConstraint constraintWithItem:topSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:marginTop]];
    [bottomSpacer addConstraint:[NSLayoutConstraint constraintWithItem:bottomSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:marginTop]];
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        if (idx > 0 ) {
            // Has previous
            NSLayoutConstraint *padding = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:subviews[idx - 1] attribute:NSLayoutAttributeBottom multiplier:1.f constant:18.f];
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? CYPMBDefaultPadding : 0.f;
...
- (void)setMode:(CYPMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == CYPMBProgressHUDModeCustomView||self.mode == CYPMBProgressHUDModeRotationView) {
            [self updateIndicators];
        }
    }
}
...
@implementation CYPMBRoundProgressView
...
@implementation CYPMBBarProgressView
...
@implementation CYPMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = CYPMBProgressHUDBackgroundStyleBlur;
            ...
            _style = CYPMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(CYPMBProgressHUDBackgroundStyle)style {
    if (style == CYPMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = CYPMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
//        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    CYPMBProgressHUDBackgroundStyle style = self.style;
    if (style == CYPMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == CYPMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
...
@implementation CYPMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [CYPMBProgressHUD allHUDsForView:view];
    for (CYPMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable CYPMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    CYPMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == CYPMBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = CYPMBProgressHUDBackgroundStyleSolidColor;
...
@implementation CYPMBProgressHUDRoundedButton
...
- (CGSize)intrinsicContentSize {
    ...
    NSLog(@"%lu",(unsigned long)self.allControlEvents);
    if (self.allControlEvents == 0||self.titleLabel.text.length == 0)
        return CGSizeZero;
