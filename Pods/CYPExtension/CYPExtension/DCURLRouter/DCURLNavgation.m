...
+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace navgation:(UINavigationController*)navSelf
{
    if (!viewController) {
        return;
        ...
        [navSelf pushViewController:viewController animated:animated];
//        [[CYPStatistic shareStatistic] statisticWithString:nil parameterDic:nil evenType:1];
//
//        [CYPStatistic shareStatistic].refferUrl = viewController.originUrl.absoluteString;
    }
}

+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace
{
    [self pushViewController:viewController animated:animated replace:replace replaceVCNums:1];
}

+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace replaceVCNums:(NSInteger)vcNumbers
{
    if (!viewController) {
        return;
        NSAssert(0, @"请添加与url相匹配的控制器到plist文件中,或者协议头可能写错了!");
        ...
        if([viewController isKindOfClass:[UINavigationController class]]) {
            ...
            if (navigationController) { // 导航控制器存在
                ...
                if (replace) {
                    ...
                    NSArray *viewControllers = [navigationController.viewControllers subarrayWithRange:NSMakeRange(0, navigationController.viewControllers.count-vcNumbers)];
